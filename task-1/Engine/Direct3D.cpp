...
void CDirect3D::DrawText(const CHAR *strText, LPRECT rect, D3DCOLOR color, int iCount)
{
	if (!m_pFont || !strText) return;
	m_pFont->DrawTextA(NULL, strText, iCount, rect?rect:&m_rWnd, DT_LEFT, color);
}
...
void CDirect3D::PreRender(DWORD flag)
{
	...
	m_pD3D9Device->Clear(0, NULL, flag, D3DCOLOR_ARGB(0xFF, 0x00, 0x00, 0x00), 1.0f, 0);
